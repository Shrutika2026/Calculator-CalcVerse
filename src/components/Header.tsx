...
export default function Header({ darkMode, toggleDarkMode, activeTab, setActiveTab, soundEnabled, toggleSound }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b bg-card-bg border-border-main/80 select-none shadow-sm transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-6 md:px-8 py-2 md:py-1.5 flex flex-col items-center gap-2 md:gap-0.5">
        ...
        <div className="w-full flex items-center justify-between">
          ...
          >
            ...
            <div className="flex flex-col">
              <h2 className="text-[10px] sm:text-sm md:text-2xl font-extrabold leading-tight tracking-[0.12em] uppercase truncate max-w-[120px] sm:max-w-[160px] md:max-w-none text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 via-violet-500 to-indigo-700 drop-shadow-lg">
                CALCVERSE
              </h2>
          ...
          <div className="flex items-center space-x-4 justify-end">
            <div className="flex items-center gap-3">
              <div className="flex flex-col text-right pr-2 min-w-[80px]">
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <span className="absolute bottom-[-14px] left-0 right-0 h-[3px] bg-brand-accent rounded-full animate-fade-in shadow-[0_-1px_6px_rgba(16,185,129,0.6)]" />
                  )}
                </button>
              );
            })}
